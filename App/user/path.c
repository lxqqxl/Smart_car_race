...
uint8 If_straight(uint8 start,uint8 end,uint8 val)
{
	...
	{
		line_w=abs_sub(Pick_table[line],214/2);
		if(line_w<=val)
			count++;
...
void with_left_to_center()
{
  ...
  for(line=START_Y;line<=valid_line;line++)
  {
    Pick_table[line]=Bline_left[line]-(fill_line)+20;//20
  }
...
void with_right_to_center()
{
  ...
  for(line=START_Y;line<=valid_line;line++)
  {
    Pick_table[line]=Bline_right[line]+(fill_line)-25;//20
  }
...
void bDistance()
{
  ...
  if(valid_line-START_Y>0)//分母不能为0（有可能卡死）
  {
      av_fact_w=total_line_w/(valid_line-START_Y);// 坡道 按照宽度检测
      if(av_fact_w>76)//坡道检测 宽度
      {
            ramp_flag=1;
      }
  }  
  if(zhang_ai_num>7&&valid_line>30)
  {
      gl_zhangai_flag=1;
  }
  if(shizi_count>8)
...
void If_LStraight()
{
   gl_zhidao_count=If_straight(START_Y,valid_line,17);//返回偏离误差范围内的行数  15 
...
int beleved_shizi_loop()
{
    uint16 line;
    int black_num=0;
    for(line=START_Y;line<58;line++)
    {
        if(center_pos[line]!=0)
        {
            if(get_img_point(line,center_pos[line]))
            {
                black_num++;
            }                
        }          
...
int loop_black_num()//解决圆环  十字 误判
{
//  int i=0,ret=0; 
//    int lie_num=40,hang_num=0;
//    for(i=hang_num;i<50;i++)
//    {
//      if(center_pos[i]!=0&&get_img_point(i,lie_num))
//      {
//      
//      }
//        if(get_img_point(i,lie_num))
//        {
//            ret=i;
//            break;
//        }
...
void shizi_find_line()
{
  int distance_b=0;
    ...
    distance_b=loop_black_num();
     
  //    uart_putchar (UART4, distance_b);
//    if(sz_end_line<51)
//	 if(distance_b<35&&distance_b>5)
      if(PTE12_IN==1)
      {
    if(distance_b<44&&distance_b>6)//40 6
	{
		LoopFlag=1;//环路
		return ;
	}
        ...
        if(distance_b<38&&distance_b>6)//40 6
	{
		LoopFlag=1;//环路
		return ;
	}
...
void get_even_diff_s()
{
  ...
  uint8 v_line1=0;//清零
  float diff1=0;
  uint16 valid_line2=diff_valid_line;//记录用多少行计算误差，大小需要调试
  uint8 center_val=107;//中心值是98，因为之前硬件上有偏差，所以-5抵消掉，根据实际情况测(右加左减)
  int16  w1,w2;
  int mix_tance=MIX_distance;  
  ...
  {    
     ...
     {
       mix_tance = 5;
       if(valid_line2-mix_tance<8||mix_tance>valid_line2)
       { if(PTE7_IN)
             even_diff=33;
         else even_diff=-33;//圆环没有道路直接左转
             
             return;
       }
     }
     else if(valid_line2<=mix_tance)//有效行数小于最近计算行
     {
        mix_tance=START_Y;
     }
