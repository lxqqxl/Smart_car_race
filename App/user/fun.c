...
void ti_jiaozheng(uint8 start,uint8 end)
{
  ...
  for(line=start;line<=end;line++)
  {
	   //////////////////////////////////左边线矫正////////////////////////////////
		...
		temp_left=(int)((Bline_left[line]-40)*(float)down_x/((line-b)/k)+107);  
		if(temp_left<0){
			Bline_left[line]=0;
			Deal_flag[line] |= DEAL_LEFT_LOST;
		}
		else if(temp_left>(214-1))
		{
			Bline_left[line]=(214-1);
			Deal_flag[line] |= DEAL_LEFT_LOST;
		}
		else
		{
		...
		temp_right=(int)((Bline_right[line]-40)*(float)down_x/((line-b)/k)+107);
		if(temp_right<0)
		{
			Bline_right[line]=0;
			Deal_flag[line] |= DEAL_RIGHT_LOST;
		}
		else if(temp_right>(214-1))
		{
			Bline_right[line]=(214-1);
			Deal_flag[line] |= DEAL_RIGHT_LOST;
		}
		else
		{
...
//   LCD_num(site,b_point_num, BLUE,RED);
    if(b_point_num>70)
    {
      stop_car_line=1;
      return 1;    
...
***********************************************************/
/*
int mark_stop()//用前十行  采取 黑白点
{
  int ret = 0,num_line = 0,num_lie=0;
  int start_line = 1, end_line = 10;
  int start_lie=0+20,end_lie=80-20;
  int b_point_num=0,w_point_num=0;
  float k=0.0;
  float av_b_num=0,av_w_num=0;
  for(num_line=start_line;num_line<end_line;num_line++)//行扫描
  {
    for(num_lie=start_lie;num_lie<end_lie;num_lie++)
    {
      if(get_img_point(num_line,num_lie))//(行，列)  收集黑点
         b_point_num++;
      else              //收集白点
         w_point_num++;
    }  
  }
//  av_b_num=(float)b_point_num/(float)num_line;
//  av_w_num=(float)w_point_num/(float)num_lie;
//  k=((float)(av_b_num))/((float)(av_w_num));
//  if(k>3.5&&k<6)
//  {
//    ret=1;
//  }
  if(b_point_num>80&&w_point_num>80)
  {
    ret=1;
  }
  return ret;
...
{ int i;
	int count=500;
	if(num>count&&flag&&valid_line<20)//停车条件成立
	{
		pidl.setPoint=0;
		pidr.setPoint=0; flag_start=1;
        stop_car_flag=1;
  	}
	if(num<count&&flag&&valid_line<10)//如果条件不成立说明刚发车  此时 在长直道上面 加速
	{
//		for(i=0;i<50;i++)
//                  Pick_table[i]=110;
//                valid_line=50;
...
int check_and_stop1()
{
  ...
  int right_x=0;
  int av_x=0;
  int start_lie=0+20,end_lie=80-20,lie=0;
  int b_num=0;
  float k=0;
  for(line_num=0;line_num<59;line_num++)
  {
      if(Bline_left[line_num]!=40&&Bline_left[line_num]-1>15)//边界跳变点检测
      {
        left_x=line_num;
        break;
      }        
  ...
  for(line_num=0;line_num<59;line_num++)
  {
    if(Bline_right[line_num]!=40&&abs(Bline_right[line_num]-79)>15)
      {
        right_x=line_num;
        break;
      }  
  }
  av_x=(left_x+right_x)/2;
  if(right_x>0&&left_x>0)
  {
    for(lie=start_lie;lie<end_lie;lie++)
    {
        if(get_img_point(av_x,lie))
        {
            b_num++;
        }    
    }
    k=(float)b_num/(float)(end_lie-start_lie);
    if(k>0.4&&k<1.6)
    {
      ret=1;
    }  
