...
//还每被用                                                         
void ring_chose_init()//用于环路选择的初始化（拨码开关）
{//最多支持四个环路
  ...
  //gpio_get(PTE10)==1;//获取拨码开关的值
}
void motor_dir_init()//D5 D6 A7 A5 (方向 脉冲 方向 脉冲)
{/************************************************************************
  配置电机方向
  */////////////////////////////////////////
...
void  main(void) 
{
  //！！！！！****一定要先读my_cfg.h中的注释**·***！！！！！！
    ...
    total_time=0;//记录运行次数，起跑线检测时用到，用于跑一段时间在检测起跑线  
    //gpio_init(PTD9,GPO,0);
     //gpio_init(PTC14,GPO,0);
     gpio_init(PTE7,GPI,0);
     gpio_init(PTE12,GPI,0);
    //motor_dir_init();
    //change_angle(3670);//舵机中立位置（用于测试）
    while(1)
